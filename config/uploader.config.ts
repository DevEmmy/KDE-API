...
export const fileUploader = multer({
  storage: multer.diskStorage({
    destination(req: Request, file: Express.Multer.File, cb: MulterStorageCallback) {
      cb(null, path.join(__dirname, '../images'));
    },

    filename(req: Request, file: Express.Multer.File, cb: MulterStorageCallback) {
...
export const uploadToCloud = async (file: Express.Multer.File, options?: UploadApiOptions): Promise<string> => {
  try {
...
export const uploadListingMedia = async (media: Base64Media[] | any[], option?: UploadApiOptions) => {
  let i = 0;
  const ims = [];
